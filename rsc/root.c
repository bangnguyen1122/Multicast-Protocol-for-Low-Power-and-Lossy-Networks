...
#define MAX_PACKETS 51  /* number of packets to send */
#define DUPLICATE_PROBABILITY 70  /* create duplicate */
...
multicast_send(void)
{
  ...
  /* send duplicate with probability DUPLICATE_PROBABILITY% */
  if((random_rand() % 100) < DUPLICATE_PROBABILITY) {
    uip_udp_packet_send(mcast_conn, buf, sizeof(id));
    PRINTF("** Duplicate sent for msg=0x%08lx **\n",
           (unsigned long)uip_ntohl(*((uint32_t *)buf)));
  }
...
PROCESS_THREAD(rpl_root_process, ev, data)
{
  ...
  random_init(clock_time()); /* avoid same random sequence in each run */
